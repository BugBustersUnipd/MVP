class AiGeneratorJob < ApplicationJob
  ...
  def perform(generation_id)
    AiJobOrchestrator.signal_process_start(generation_id)
    
    begin
      service = AIGeneratorContainer.new.aiGeneratorService
      service.create_content(generation_id)

      AiJobOrchestrator.complete(generation_id)
      ...
      AiJobOrchestrator.signal_failure(generation_id, e.message)
    end
  end
end