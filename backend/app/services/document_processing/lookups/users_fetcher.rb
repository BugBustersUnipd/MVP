  ...
  module Lookups
    class UsersFetcher
      ...
      def call(company: nil)
        ...
        # `matched_employee_id` in extracted_documents now references `users.id`.
        # Map back to Employee records via `user_id` for compatibility with callers.
        Employee.where(user_id: matched_ids)
      end
    end
  end
end